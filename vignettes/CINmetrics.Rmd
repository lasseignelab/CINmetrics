...
Alternatively, you can download the data from TCGA using TCGAbiolinks package
```{r}
## Not run:
...
$$
```{r}
modified.tai.test <- taiModified(cnvData = maskCNV.BRCA)
...
$$
Total\ Copy\ Number\ Abberation = \sum^{R}_{i = 1} n_i
\ \ where\ \ 
\begin{align}
...
```{r}
break.points.test <- countingBreakPoints(cnvData = maskCNV.BRCA)
...
```{r}
fraction.genome.test <- fga(cnvData = maskCNV.BRCA)
